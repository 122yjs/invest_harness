...
<!-- GENERATED-SYNC: source=plugins/vertical-plugins/invest-research/skills/report-synthesizer/SKILL.md; kind=skill; script=scripts/sync_invest_skills.py -->
> [!IMPORTANT]
...
### Findings → Report 변환 규칙
- findings의 표/수치를 단순 복사하지 말고, 정성적 내러티브로 재구성할 것
- 각 섹션에 "So What" 문단을 포함할 것
- 논리 흐름: "무엇인가(What)" → "왜 중요한가(Why)" → "투자적으로 무슨 의미인가(So What)"
- Executive Summary, 핵심 투자 포인트, Risk-Reward, 시나리오 투자 함의 등에서 정성적 분석을 중심으로 작성할 것
- findings에 없던 투자 결론이나 매매 의견을 새로 도출하지 말 것 (findings 기반 해석만 추가)
- "~입니다", "~합니다" 체를 사용하고, 과장된 수식어와 구어체를 피할 것

> **원칙**: Findings는 "무엇인가"를 담당하고, Report는 "그래서 무엇인가"를 담당한다.
