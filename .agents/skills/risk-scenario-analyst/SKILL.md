...
description: Part XII 리스크 분석과 Part XIII 시나리오 분석을 수행하여 _workspace/06_risk_scenario/findings.md와 conflicts.md를 정리하는 전문가 스킬
---
...
4. **충돌 및 불일치 기록**
   ...
   - 필요한 경우 `_workspace/06_risk_scenario/conflicts.md`에 충돌 목록을 작성한다.
...
6. **파일 저장**
   - 최종 결과를 `_workspace/06_risk_scenario/findings.md`에 저장한다.
   - 충돌이 있으면 `_workspace/06_risk_scenario/conflicts.md`에 별도로 저장한다.
