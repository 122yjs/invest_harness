...
- Markdown 지침 파일
- `${ACTIVE_WORKSPACE}/` 아래의 결정적 산출물 경로
...
모든 산출물은 team-spec.md의 `` 경로에 저장한다.
```
...
출력은 ${ACTIVE_WORKSPACE}/03_valuation/findings.md에 저장한다.
```
...
Apple(AAPL, NASDAQ)에 대한 표준형 투자 리포트를 생성하고 `` 산출물 계약을 지킨다.
```
...
출력은 ${ACTIVE_WORKSPACE}/01_financial/findings.md에 저장한다.
```
...
- 산출물은 `${ACTIVE_WORKSPACE}/`에 남겨 감사 가능성을 유지한다.
