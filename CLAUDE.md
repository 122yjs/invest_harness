...
Claude 또는 Claude Code 계열 에이전트는 아래 문서를 함께 읽고 작업합니다.
...
역할별 `SKILL.md` 파일은 특정 런타임 전용 코드가 아니라 Markdown 작업 지침입니다. Claude 환경에서 skill discovery가 자동으로 동작하지 않으면 사용할 역할 파일 경로를 프롬프트에 직접 명시합니다.
