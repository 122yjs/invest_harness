...
`yfinance-mcp` MCP 서버가 live runtime에서 사용 가능하면 Yahoo Finance 데이터를 API 키 없이 조회할 수 있다. 미국/글로벌 기업 분석의 주력 데이터 소스이며, 한국 기업 분석 시에도 korea-stock-mcp와 함께 보조로 사용한다.
...
사용 전에는 현재 세션의 callable source inventory를 확인한다. repo 문서상 connected인 source라도 live runtime에서 unavailable이면 `${ACTIVE_WORKSPACE}/00_evidence/source-call-plan.md`에 `Runtime Availability=unavailable`로 남기고 company IR, SEC EDGAR, DART/KRX 또는 local regulator filing을 우선 fallback으로 검토한 뒤 FMP/Alpha Vantage/Web Search + Fetch를 보조 경로로 사용한다.
...
이 README에 명시된 callable 공개 MCP는 `yfinance-mcp`와 `korea-stock-mcp`이다. 추가 source는 `docs/harness/invest/research-layer/source-capability-registry.md`의 `connection_status`로 판단한다. 단, `connection_status`는 repo-evidence 상태이며 live runtime proof가 아니다.

Evidence 신뢰도는 live runtime availability와 분리한다. Company IR, SEC EDGAR, DART/KRX, local regulator filing은 T0 official disclosure로 취급하고 reported financial fact의 우선 근거로 사용한다. yfinance/FMP/Alpha Vantage 같은 vendor snapshot은 T2 cross-check 또는 T0 부재 시 보조 근거이며, T0 공식 공시를 덮어쓰지 않는다.
...
- 최근 분기 재무 비교와 peer financial comparison에는 가능한 경우 YoY와 QoQ를 함께 제공합니다.
