...
<!-- GENERATED-SYNC: source=plugins/vertical-plugins/invest-research/skills/html-report-synthesizer/SKILL.md; kind=skill; script=scripts/sync_invest_skills.py -->
> [!IMPORTANT]
...
2. **100% 한글화 원칙 준수**:
   ...
   - HTML에 포함되는 Implementation Plan, Task, Walkthrough, 원본 마크다운 뷰어 제목, Toast, QA 배지, 오류 메시지도 모두 한국어로 작성한다.
   - 티커, 회계 약어, 차트 라이브러리명처럼 필요한 고유명사는 영어를 유지할 수 있지만, 사용자의 해석을 돕는 설명은 한국어로 제공한다.
...
5. **심층 내러티브 및 검증 로그 시각화 (02, 05, 06 파트)**:
     ...
     - 내러티브는 `합니다/습니다`체로 작성하고, 비개발자 재무·회계 실무자도 이해할 수 있게 전문 용어와 산식을 쉬운 한국어로 풀어쓴다.
     - `혁신적인`, `획기적인`, `놀라운`, `완벽한`, `최고 수준` 같은 과장된 수식어와 `~요`, `~죠` 같은 구어체 어미를 사용하지 않는다.
     - 투자 리포트 HTML에는 블로그용 인사말인 `안녕하세요, PROCPA입니다.`나 다음 글 예고를 넣지 않는다.
...
7. **글래스모피즘 프리미엄 CSS 테마 적용**:
   ...
   - Harmonious HSL 컬러 세트, Inter/Outfit 및 Noto Sans KR 서체, 부드러운 호버 전환 효과, 그라데이션 보더 라인 등을 적용하여 정보 밀도와 가독성의 균형을 맞춘다.
...
8. **원본 MD 절대 경로 상시 노출 및 아코디언 상태 보존**:
   ...
   - **클립보드 원클릭 복사 및 Toast 알림**: 경로 badge 우측에 '경로 복사' 버튼을 배치하고, 성공 시 화면 우측 하단에 에메랄드 HSL 컬러 기반의 Toast 팝업 알림이 2초간 등장하도록 동적 인터랙션을 연동한다.
...
- **문체 검증**: HTML 화면 텍스트와 인라인 렌더링된 원본 Markdown이 `report-writing-style-policy.md`의 한국어, 마크다운, PROCPA 기반 실무형 문체 규칙을 따르는지 확인한다.
